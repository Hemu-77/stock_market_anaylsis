...
async function renderList() {
  ...
  for (const stock of list) {
    ...
    const nme = document.createElement("button");
    nme.classList.add("list");
    ...
    nme.textContent = stock;
    nme.value = stock;
    bookV.textContent = `$${bookValue}`;
    bookV.style.color = 'white';
    proft.textContent = `${profit.toFixed(2)}%`;

    list_item.append(nme, bookV, proft);
    listEl.append(list_item);
    nme.addEventListener("click", (e) => {
